impl Solution {
    ...
    ///
    /// # Intuition
    /// Once `arr1[i]` is chosen, `arr2[i]` is fixed as `nums[i] - arr1[i]`. The monotone
    /// constraints become a lower bound on how fast `arr1` must increase between indices.
    ///
    /// # Approach
    /// - Let `a_i = arr1[i]` with bounds `0 <= a_i <= nums[i]`.
    ...
    /// - Dynamic programming over each index keeps counts for each possible `a_i`.
    ///   Use a prefix sum to compute transitions in O(1) per value.
    ///
    /// # Complexity
    /// - Time: O(n * max(nums))
    /// - Space: O(max(nums))
