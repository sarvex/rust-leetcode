impl Solution {
    /// Prefix sum approach with early termination
    ///
    /// # Intuition
    ...
    /// 2. For each size k from largest down to 2, enumerate all positions
    /// 3. Compare first row vs first column immediately to prune fast
    /// 4. Then check diagonals before remaining rows/columns
    ///
    /// # Complexity
        ...
        for edge in (2..=m.min(n)).rev() {
            for i in 0..=m - edge {
                'inner: for j in 0..=n - edge {
                    ...
                    // Quick check: first column must match first row
                    ...
                    // Check remaining rows (skip first)
                    for ii in 1..edge {
                    ...
                    // Check remaining columns (skip first)
                    for jj in 1..edge {
...
mod tests {
    ...
    #[test]
    fn test_no_magic_square_larger_than_1() {
        let grid = vec![vec![1, 2], vec![3, 4]];
        assert_eq!(Solution::largest_magic_square(grid), 1);
    }

    #[test]
    fn test_2x2_magic_square() {
