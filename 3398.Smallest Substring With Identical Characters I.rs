impl Solution {
    /// Finds minimum longest run after at most num_ops character flips.
    ///
    /// # Intuition
    /// The answer is monotonic: achieving max-run k implies any k' > k is also
    /// achievable. Binary search on the answer, with O(n) validation per candidate.
    ///
    /// # Approach
    /// 1. Binary search on candidate max_len in [1, n].
    /// 2. For max_len = 1: count mismatches against both alternating patterns.
    /// 3. For max_len > 1: each run of length L needs ⌊L / (max_len+1)⌋ flips.
    ///
    /// # Complexity
    /// - Time: O(n log n)
